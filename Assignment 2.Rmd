...
title: "Assignment 2"
author: "Omar Amin"
contributor: "Kasra Foroughi EDITS"
date: "2022-10-28"
output:
  html_document:
    df_print: paged
...
#EDIT 1- KASRA - This edit was redone for the Rag1_Seq
#In this edit, my aim was to filter the data in mitoc_final by removing sequences that have missing data (more than 1% N in the sequences)
#Before the edit there are 35 variables for mitoc_final
nrow(mitoc_final)
missing.data <- 0.01
...
length.var <- 50
  ...
  df1 <-mitoc_final %>%
    mutate(nucleotides2 = str_remove_all(mitoc_Sequence, "^N+|N+$|-")) %>%
    filter(str_count(nucleotides2, "N") <= (missing.data * str_count(mitoc_Sequence))) %>%
...
#Need to remove the previous mitoc_Sequence column and rename nucleotides2 as mitoc_Seq for alignment 
df1_edit <- df1 %>%
  select(-c('mitoc_Sequence'))
colnames(df1_edit)[which(names(df1_edit) == "nucleotides2")] <- "mitoc_Sequence"
#After the fucntion and edit - we can see that there are now 34 variables, meaning that 1 sequence was filtered due to exceeding the 1% threshold for missing data
nrow(df1_edit)
mitoc_final <- df1_edit
...
#EDIT 1.2- KASRA - 
#In this edit, my aim was to filter the data in rag1_final by removing sequences that have missing data (more than 1% N in the sequences)
#Before the edit there are 35 variables for rag1_final
nrow(rag1_final)
  #This section filters out the data
  #It first creates a new column known as nucleotides2 which removes the terminal N's and gaps in each sequence
  #It then filters out any sequences which have more than 1% missing data = dont want too many missing sequences
  #It then filters out sequences which are outside the range of the median sequence length in accordance to length.var
  df2 <-rag1_final %>%
    mutate(nucleotides2 = str_remove_all(rag1_Sequence, "^N+|N+$|-")) %>%
    filter(str_count(nucleotides2, "N") <= (missing.data * str_count(rag1_Sequence))) %>%
    filter(str_count(nucleotides2) >= median(str_count(nucleotides2)) - length.var & str_count(nucleotides2) <= median(str_count(nucleotides2)) + length.var)
...
df2_edit <- df2 %>%
    select(-c('rag1_Sequence'))
colnames(df2_edit)[which(names(df2_edit) == "nucleotides2")] <- "rag1_Sequence"
#After the filter - we can see that 2 sequences were filtered - meaning that 2 sequences exceeded the threshold of more than 1% missing data
nrow(df2_edit)
rag1_final <- df2_edit
...
#KASRA EDIT (for Edit 3) - Part 3.1 - DistanceMatrix_Mito
#The purpose of this second edit was to create a cluster of the mitoc and rag1 genes 
#Needed to transform the mitoc_bin as a dist.dna for hclustering 
distanceMatrix.raw_MITO <- dist.dna(mitoc_bin, model = "TN93", as.matrix = TRUE, pairwise.deletion = TRUE)
```


```{r fig.width = 20, fig.height = 15}
#Don't need to plot the distance matrix if clustering anymore 
# nj_mitoc_tn93 <- nj(distanceMatrix.raw_MITO)
# plot(nj_mitoc_tn93)
```
...
#KASRA EDIT (for Edit 3) - Part 3.2 - DistanceMatrix_Rag1
#The purpose of this second edit was to create a cluster of the mitoc and rag1 genes 
#Needed to transform the mitoc_bin as a dist.dna for hclustering 
distanceMatrix.raw_Rag1 <- dist.dna(rag1_bin, model = "TN93", as.matrix = TRUE, pairwise.deletion = TRUE)

```


```{r fig.width = 20, fig.height = 15}
#Dont need a plot for distanceMatrix 
# nj_rag1_tn93 <- nj(distanceMatrix.raw_Rag1)
# plot(nj_rag1_tn93)
```
...
**KASRA EDIT 2 & 3- Clustering the MITO and RAG1 Genes
```{r}
#Edit 2 - In this section, I am trying to figure out what the best clustering method is before the final cluster of mitoc and rag1 genes
...
ac_Mito <- function(x) {
  agnes(distanceMatrix.raw_MITO, method = x)$ac
}
ac_Mito_values <- purrr::map_dbl(m, ac_Mito) 

#CYTB AC
ac_Rag1 <- function(x) {
  agnes(distanceMatrix.raw_Rag1, method = x)$ac
}
ac_Rag1_values<- purrr::map_dbl(m, ac_Rag1) 

#The best method of clustering for both genes is Ward
which.max(ac_Mito_values)
which.max(ac_Rag1_values)

#Edit 3 - This edit is to add a hierarchical clustering method for mitoc and rag1 genes 
# I have first figured out that the wWard clustering method is the best for both genes

distanceMatrix.raw_MITO = as.dist(distanceMatrix.raw_MITO)
distanceMatrix.raw_Rag1 = as.dist(distanceMatrix.raw_Rag1)

clustering.method <- "ward.D2"
Mito_Cluster <-hclust(distanceMatrix.raw_MITO,method=clustering.method)
#Plot the COI cluster 
plot(Mito_Cluster, main =  "MITO Cluster", xlab = "Species")
#Clustering CYTB
Rag1_Cluster <-hclust(distanceMatrix.raw_Rag1,method=clustering.method)
#Plot the CYTB Cluster
plot(Rag1_Cluster, main = "Rag1 Cluster", xlab = "Species")

#EDIT 3 - Determining which gene had a stronger clustering through silhouette index 
hc_cut_MITO <- hcut(distanceMatrix.raw_MITO, k = 3, hc_method = "ward.D2")
# Visualize silhouhette information
fviz_silhouette(hc_cut_MITO, main = "MITO Silhouette")
#Rag 1 has a much higer width and average values - which signifies that it is the better gene for clustering 
hc_cut_Rag1 <- hcut(distanceMatrix.raw_Rag1, k = 3, hc_method = "ward.D2")
# Visualize silhouhette information
fviz_silhouette(hc_cut_Rag1, main = "Rag1 Silhouette")
```



**Part 3: Geophylogeny analysis**
Kasra Edit - This section has been commented out as it presented problems originally and I was not able to fix it 
